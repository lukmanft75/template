...
/******************************************************************************
    ...
    select_box($name,$caption,$values,$selecteds=array(),$width,$height,$z_index=0,$maxselects=5,$border_color="#43579C",$title_color="#000",$contains_color="grey",$backcolor="white",$attr="",$class="")
 ******************************************************************************/
...
class FormElements extends Database {
    ...
    public function select_box($name,$title,$values,$selecteds=array(),$width=280,$height=200,$z_index=50,$maxselects=5,$title_height=25,$font_size=12,$title_color="#000",$border_color="grey",$contains_color="grey",$backcolor="white",$attr="",$class=""){
        $arr_elms = "";
        ...
        $return ='
            ...
            <div style="text-align:left;position:relative;width:'.($width+17).'px;height:5px;z-index:'.$z_index.';">
                <div onmouseover="try{ hiding_select_box(\'select_box_values_'.$name.'\'); } catch(e){}" style="font-family: Ubuntu; display: flex;align-items: center;width:'.($width+17).'px;font-size:'.($font_size+1).'px;height:'.($title_height+1).'px;border:solid '.$border_color.' 1px;position:absolute;z-index:'.$z_index.';background-color:'.$backcolor.';">
                    <div style="margin:0px 0px 0px 5px;float:left;width:'.($width).'px;color:'.$title_color.';" id="select_box_'.$name.'" onclick="select_box_toggle_'.$name.'();">
						<table width="100%"><tr>
							...
							<td align="right" style="background-color:white"><div style="position:relative;left:5px;margin:0px 5px 0px 0px;float:right;cursor:pointer;background-image: url(icons/arrow_down.png);height:17px;width:17px;"></div></td>
						</tr></table>
					</div>
                </div>
    ...
    public function select_box_ajax($name,$title,$selecteds=array(),$width,$height,$z_index=0,$maxselects=5,$title_height=12,$font_size=12,$title_color="#000",$border_color="#43579C",$contains_color="grey",$backcolor="white",$attr="",$class=""){
	...
	public function select_window($name,$title,$value="",$tablename,$id_field,$caption_field,$window_filename = "win_default.php"){
		$window_filename = "sub_window/".$window_filename."?name=".$name."&value=".$value."&title=".$title."&tablename=".$tablename."&id_field=".$id_field."&caption_field=".$caption_field;
		if($value != "") {
			if(strpos(" ".$value,"|") > 0){
				$caption = "";
				$values = pipetoarray($value);
				foreach($values as $val){
					$caption .= "- ".$this->fetch_single_data($tablename,$caption_field,array($id_field => $val))."<br>";
				}
			} else {
				$caption = $this->fetch_single_data($tablename,$caption_field,array($id_field => $value));
			}
		}
		$return = $this->input($name,$value,"type='hidden'");
		$return .= "<div style='position:relative;float:right;'><img src='icons/search_window.png' onclick='$.fancybox.open({ href: \"".$window_filename."\", height: \"80%\", type: \"iframe\" });'></div>";
		$return .= "<div id='sw_caption_".$name."' style='font-style: italic;font-weight: bold;'>".@$caption."</div>";
		return $return;
	}
}
?>
